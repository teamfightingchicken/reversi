	...
	*/
	...
	socket.on('invite',function(payload){
		...
		if(!roomObject.sockets.hasOwnProperty(requested_user)){
			var error_message = 'invite requested a that wasn\'t in the room, command aborted';
			log(error_message);
			socket.emit('invite_response', {
				result: 'fail',
				message: error_message
...
//should this be changed to uninvite?
		var room = players[socket.id].room;
		var roomObject = io.sockets.adapter.rooms[room];
		/* Make sure the user being invited is in the room*/
		if (!roomObject.sockets.hasOwnProperty(requested_user)) {
		...
		/* Tell the uninvitee they have been invited*/
	...
	*/
	...
	socket.on('game_start', function(payload) {
		...
		/* Make sure the user being invited is in the room*/
		if (!roomObject.sockets.hasOwnProperty(requested_user)) {
			var error_message = 'game_start requested a that wasn\'t in the room, command aborted';
			log(error_message);
			socket.emit('game_start_response', {
				result: 'fail',
				message: error_message
	...
	if success a success message by a game_update message
	play_token_response:
	}
	or {
		'result': 'fail',
	...
	*/
	...
	socket.on('play_token', function (payload) {
		...
		if (('undefined' === typeof game_id) || !username) {
		...
		if (('undefined' === typeof gmame) || !game){
...
function send_game_update(socket, game_id, message){
	/* Check to see if a game_id already exists*/
	...
	if ((games[game_id].player_white.socket != socket_id) && (games[game_id].player_black.socket != socket_id)){
		console.log('Player isn\'t assigned a color: '+socket_id);
		/* and there isn't a color to give them */
		if ((games[game_id].player_black.socket != '') && (games[game_id].player_black.socket != '')){
